...
markChallengeAsWaitingForReviewController.post(
  ...
  zValidator(
    "json",
    markChallengeAsWaitingForReviewUseCaseSchema,
    (result, c) => {
      if (!result.success) {
  ...
  createMiddleware<Env>(async (context, next) => {
    ...
    context.set(
      "markChallengeAsWaitingForReviewUseCase",
      new MarkChallengeAsWaitingForReviewUseCase(challengeRepository),
    );
  ...
  async (context) => {
    ...
    const payload =
      await context.var.markChallengeAsWaitingForReviewUseCase.invoke(body);
